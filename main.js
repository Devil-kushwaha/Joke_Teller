...
async function getJokesFromWeb(url) {
    try{
...
function filterJoke(getData){
    if(getData.setup){
        joke = `${getData.setup}  ...  ${getData.delivery}`;
    }
    else{
        joke = getData.joke;
    }
...
const processJoke = async () =>{
    const getUrl =  makeUrl();
    const getData = await getJokesFromWeb(getUrl);
    const joke = await filterJoke(getData);
    // Text-to-Speech
    tellMe(joke);
}
 

button.addEventListener('click',processJoke);
// Re-enbling the button after audio is ended(Note:button.disabled=false(default))
